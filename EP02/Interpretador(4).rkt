...
(define (interp [a : ExprC] [env : Env] ) : Value
  (type-case ExprC a
    ...
    ;; Olha para a box de n, transforma numa promise e calcula a promise.
    ;; Se n for uma suspV, calcula e aplica na box de n, que possivelmente é o parâmetro
    ;; O parametro que a gente colocou numa box.
    ;; Caso contrário, devolve o valor da box de n
    ...
    f pode ser um suspV
    ((lambda (x) (+ x 1)) (+ 2 3))
    Associa x a suspensão de argvalue <|(+ 2 3), env|>
    Quando x for chamado (porque ele é um IdC), ele calcula em IdC pelo query-promise
    |#
    
    [appC (f a)
          (let ((closure (interp f env)) ;; Closures não são promises
                (argvalue (suspV a env))) ;; Argvalue é uma box/ promise? Um Value? 
            (type-case Value closure
              [closV (parameter body env)
                     (interp body (extend-env (bind parameter (box argvalue)) env))]
                     ;;(interp body (extend-env (bind parameter (aPromise )) env) )]
    ...
    |#
    [carC (c) (type-case Value (interp c env)
                [consV (car cdr) ;; car e cdr são suspV
                       (query-promise car)] ;; Executa a car
                [else (error 'interp "car applied to non-cell")]
                )]
    ...
    EP02
    |#
    [equalC (l r) (let ([left (interp l env)]
                        [right (interp r env)])
                    (if (equal? left right)
                        (numV 1)
                        (numV 0)
...
;; Testes do EP2
(interpS '(equal? 1 1))
(interpS '(equal? (+ 1 2) (* 3 1)))
(interpS '(equal? (cons 3 4) (cons 4 5)))
(interpS '(equal? 'alan 'usp))
